...
function FinalContent() {
  ...
  // Notice the Fragment <> at the top level and print:hidden on the main UI wrapper
  return (
    <>
      <div className="min-h-screen bg-slate-50 flex flex-col font-sans print:hidden">
        ...
        <div className="flex flex-1 overflow-hidden max-w-[1600px] mx-auto w-full p-6 lg:p-8 gap-8 flex-col lg:flex-row">
          ...
          <div className="w-full lg:w-[400px] flex flex-col gap-4 overflow-y-auto shrink-0 pr-2 scrollbar-thin scrollbar-thumb-slate-200">
            ...
            {/* --- NEW: COMBINED PDF BUTTON --- */}
            <div className="mt-4 pt-4 border-t border-slate-200">
              <button 
                onClick={() => window.print()}
                className="w-full bg-slate-900 hover:bg-slate-800 text-white px-5 py-3.5 rounded-xl text-sm font-bold shadow-sm transition flex items-center justify-center gap-2"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                Save as PDF Document
              </button>
              <p className="text-[10px] text-center text-slate-400 mt-2 uppercase tracking-wider">
      ...
      <div className="hidden print:block p-8 bg-white text-black w-full max-w-4xl mx-auto">
        <div className="border-b-2 border-slate-900 pb-4 mb-8">
          <h1 className="text-4xl font-black text-slate-900">Event Master Document</h1>
          <p className="text-slate-500 mt-2 font-mono">{sessionId?.replace("session-", "Event ID: ") || "Generated by Event Hive AI"}</p>
        </div>

        <div className="mb-10 break-inside-avoid">
          <h2 className="text-2xl font-bold border-b border-slate-300 pb-2 mb-4 text-emerald-800">Marketing Copy</h2>
          <div className="prose prose-sm max-w-none text-slate-800"><ReactMarkdown>{outputs.marketingOutput || "No content generated."}</ReactMarkdown></div>
        </div>

        <div className="mb-10 break-inside-avoid">
          <h2 className="text-2xl font-bold border-b border-slate-300 pb-2 mb-4 text-blue-800">Email Campaign</h2>
          <div className="prose prose-sm max-w-none text-slate-800"><ReactMarkdown>{outputs.mailingOutput || "No content generated."}</ReactMarkdown></div>
        </div>

        <div className="mb-10 break-inside-avoid">
          <h2 className="text-2xl font-bold border-b border-slate-300 pb-2 mb-4 text-purple-800">Event Timeline</h2>
          <div className="prose prose-sm max-w-none text-slate-800"><ReactMarkdown>{outputs.schedulerOutput || "No content generated."}</ReactMarkdown></div>
        </div>

        <div className="mb-10 break-inside-avoid">
          <h2 className="text-2xl font-bold border-b border-slate-300 pb-2 mb-4 text-teal-800">WhatsApp Invite</h2>
          <div className="prose prose-sm max-w-none text-slate-800 whitespace-pre-wrap"><ReactMarkdown>{outputs.whatsappOutput || "No content generated."}</ReactMarkdown></div>
        </div>
      </div>
    </>
  )
